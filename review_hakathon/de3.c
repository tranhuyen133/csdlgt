...
// Danh sách liên kết đôi: đơn hàng chưa giao
Order *head = NULL, *tail = NULL;

// Danh sách liên kết đơn: đơn hàng đã giao
...
// ======= HIỂN THỊ DANH SÁCH ĐƠN HÀNG (CẢ HAI DANH SÁCH) =======
void displayOrders() {
    printf("\n---- Don hang CHUA GIAO ----\n");
    Order *cur = head;
...
// ======= ĐÁNH DẤU ĐÃ GIAO: chuyển sang danh sách đơn =======
void markAsDelivered() {
    ...
    // Tạo node mới cho danh sách đơn đã giao
...
// ======= SẮP XẾP ĐƠN HÀNG CHƯA GIAO THEO TỔNG TIỀN (TĂNG DẦN) =======
int parseMoney(char *total) {
    return atoi(total); // đơn giản hóa: giả sử là số nguyên
}
